-- ============================================================================
-- 026: マーケットの「売れた」通知に受取額を追加
-- 001〜025 の後に、SQL Editorで実行してください。
--
-- 仕様の要点:
--   ・market_sale_notificationsのtotal_priceは「売れた金額(取引額)」、
--     新設のreceived_amountは「実際に出品者が受け取った額」を表す。
--     現状は手数料が無いため常に同額になるが、将来手数料を復活させる
--     場合にも通知の意味が変わらないよう、あえて別の列として持つ。
--   ・受取額が確定するのはmarket_buy_item/market_buy_stack(購入時に
--     出品者へ入金する処理)のみなので、この2つの中身だけを差し替える。
-- ============================================================================
...
-- ----------------------------------------------------------------------------
-- market_buy_item: 「売れた」通知にreceived_amountを記録する
-- ----------------------------------------------------------------------------
...
begin
  ...
  values (v_listing.seller_id, 'equipment', 'sold', v_listing.item_snapshot->>'name', 1, v_listing.price_per_unit, v_listing.price_per_unit, v_listing.price_per_unit);
...
-- ----------------------------------------------------------------------------
-- market_buy_stack: 消費した出品(行)ごとの「売れた」通知にreceived_amountを記録する
-- ----------------------------------------------------------------------------
...
begin
  ...
  loop
    ...
    values (v_row.seller_id, 'stack', 'sold', v_row.bucket, v_row.material_key, v_take, p_price_per_unit, v_take * p_price_per_unit, v_take * p_price_per_unit);
