-- ============================================================================
-- 021: 七彩の塔(エンドコンテンツ)の突入状態
-- 001〜020 の後に、SQL Editorで実行してください。
--
-- 仕様:
--   ・七彩の塔は通常ダンジョンとは別枠のエンドコンテンツ。突入中の状態
--     (どの塔の何階にいるか)をplayers.tower_run(jsonb、{towerId, floor}
--     形式)で持つ。非アクティブ(未突入・クリア/リタイア済み)ならnull。
--   ・塔の定義(敵・ボス・報酬装備など)はすべてクライアント側
--     (index.html のTOWER_DEFS)に定義があり、DB側は突入状態の永続化のみ。
--     報酬として得る限定装備・素材は既存のplayer_inventory/players.materials
--     にそのまま乗るため、別テーブルは不要。
--   ・突入・進行・撤退・全滅時の報酬確定はすべてクライアント側で完結させ、
--     他の状態同様save_full_stateの通常保存に乗せる(スタミナ・オーブ・
--     素材・道具と同じ信頼モデル)。新規のRPCは追加しない。
--   ・save_full_stateの引数が1つ増える(p_tower_run)ため、既存の19引数版を
--     明示的に破棄してから作り直す(004・006・009・017・018・019・020と
--     同じ理由)。
-- ============================================================================

alter table public.players
  add column if not exists tower_run jsonb;

drop function if exists public.save_full_state(
  text, integer, jsonb, jsonb, integer, integer, timestamptz, jsonb, jsonb, jsonb, integer, integer, jsonb, boolean, jsonb, jsonb, integer, timestamptz, jsonb
);
...
begin
  ...
  insert into public.players (id, name, gold, party, home_furniture,
    ...
    stamina, stamina_updated_at, tools, tower_run, updated_at)
  values (v_player_id, p_name, p_gold, p_party, p_home_furniture,
    ...
    coalesce(p_tools, '{}'::jsonb), p_tower_run, now())
