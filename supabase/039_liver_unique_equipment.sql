-- ============================================================================
-- 039: 固有装備(ライバー専用装備)
-- 001〜038 の後に、SQL Editorで実行してください。
--
-- 内容:
--   ・player_inventoryにunique_for_liver_id列(text)を追加。固有装備
--     (レアリティ「ユニーク」)にのみ値が入り、そのライバーのIDを保持する
--     (通常の装備はnullのまま)。
--   ・save_full_stateは引数を増やさず(038から不変)、既存のp_inventoryの
--     各要素にunique_for_liver_idキーが増えた分だけ読み書きする列を追加する。
-- ============================================================================

alter table public.player_inventory
  add column if not exists unique_for_liver_id text;
...
begin
  ...
  insert into public.player_inventory (id, player_id, item_type, name, rarity, value,
    ...
    effect_type, effect_value, slot_count, slot_cost, socketed_into, unique_for_liver_id)
  select
    (item->>'id'),
    v_player_id,
    (item->>'item_type'),
  ...
  on conflict (id) do update set
    ...
    unique_for_liver_id = excluded.unique_for_liver_id;
